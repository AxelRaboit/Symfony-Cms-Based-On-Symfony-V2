...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Variables
    const searchForm = document.getElementById('search-page-type');
    const containerResetFormButton = document.getElementById('container-reset-button-js');
    const resetFormButton = document.getElementById('refresh-page-type-list-js');
    const searchFormInput = document.getElementById('search-page-type-input-js');
    const suggestionsList = document.getElementById('search-page-type-suggestions');
    ...
    // This function is used to display the reset button when the user click on it, it will reset the search form and display all page types
    // This button is called "Afficher tous les types de page"
    ...
    handleInputEvent(searchFormInput, function (event) {
        ...
        if (searchTerm.length >= 1) { // Begin search only if search term is at least 1 characters long
            fetch(`/backend/admin/content/page-type/ajax-search?term=${searchTerm}`)
                .then(response => {
                    if (!response.ok) {
